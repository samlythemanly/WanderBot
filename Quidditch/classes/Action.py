	...
	def update(self):
		...
		if self.preparing:
			if self.current_step < self.duration:
				self.current_step += 1
				return
			# If the current_step is equal to duration, that means we should have executed this action already and we should reset this action and trigger the cooldown
			if self.current_step == self.duration:
